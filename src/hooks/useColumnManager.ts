...
export interface UseColumnManagerReturn {
  ...
  reorderColumns: (sourceIndex: number, destinationIndex: number, sourceList: 'visible' | 'hidden', destinationList: 'visible' | 'hidden') => void;
...
export function useColumnManager(): UseColumnManagerReturn {
  ...
  const reorderColumns = useCallback((
    ...
    setColumns(prev => {
      ...
      const destinationArray = destinationList === 'visible' ? visibleCols : hiddenCols;
  ...
  const addCustomColumn = useCallback((column: Omit<ColumnDefinition, 'id' | 'type'>) => {
  ...
  const removeCustomColumn = useCallback((columnId: string) => {
    setColumns(prev => prev.filter(col => !(col.id === columnId && col.type === 'custom')));
  }, []);
